<x-app-layout>
    <style>
        ...
        .phase-card { border-radius: 20px; border: none; margin-bottom: 2rem; box-shadow: 0 10px 25px rgba(0,0,0,0.02); transition: 0.3s; background: white;}
        .phase-icon { width: 54px; height: 54px; border-radius: 16px; display: flex; align-items: center; justify-content: center; color: #5c60f5; background-color: #eef0ff; margin-right: 18px; font-size: 1.4rem;}
        .label-custom { font-weight: 800; color: #4a5568; font-size: 0.7rem; text-transform: uppercase; letter-spacing: 1.2px; margin-bottom: 0.8rem; display: block; }
        .form-control-custom { border-radius: 15px; border: 1px solid #e2e8f0; padding: 15px; background-color: #f8fafc; transition: all 0.2s ease-in-out; font-size: 0.95rem; line-height: 1.6;resize: vertical; /* Biar guru bisa narik kotaknya ke bawah */}
        .form-control-custom:focus { background-color: white; border-color: #5c60f5; box-shadow: 0 0 0 5px rgba(92, 96, 245, 0.08); outline: none; }
        .btn-indigo-outline {  border: 2px solid #5c60f5; color: #5c60f5; font-weight: 700; border-radius: 12px; transition: 0.3s; padding: 8px 20px; font-size: 0.85rem}
        .btn-indigo-outline:hover { background-color: #5c60f5; color: white; }
        .bottom-bar { position: sticky; bottom: 0; padding: 25px 0; background: rgba(255, 255, 255, 0.9); backdrop-filter: blur(12px); border-top: 1px solid #e2e8f0; z-index: 100; }
        ...
        <form action="{{ route('sessions.store') }}" method="POST" id="pblForm">
            ...
            <div class="bottom-bar">
                <div class="container d-flex justify-content-center">
                    <button type="submit" class="btn bg-indigo text-white btn-lg px-5 fw-bold py-3 shadow-lg hover-scale" style="border-radius: 18px; transition: 0.3s;">
